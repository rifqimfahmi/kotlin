...
import org.jetbrains.kotlinx.atomicfu.compiler.backend.buildFunctionSimpleType
...
class AtomicSymbols(
    ...
    val javaLangObject: IrClassSymbol = createClass(javaLang, "Object", ClassKind.CLASS, Modality.FINAL)
    ...
    val atomicRefNewUpdater: IrSimpleFunctionSymbol =
        atomicRefFieldUpdaterClass.owner.addFunction(
            ...
            addValueParameter("tclass", javaLangClass.starProjectedType)
            addValueParameter("vclass", javaLangClass.starProjectedType) // todo pass casted type that is passed as type arg forr AtomicRef
            addValueParameter("fieldName", irBuiltIns.stringType)
        }.symbol
    ...
    private val UPDATER_CLASSES: Map<IrType, IrClassSymbol> = mapOf(
        irBuiltIns.intType to atomicIntFieldUpdaterClass,
        irBuiltIns.longType to atomicLongFieldUpdaterClass,
        irBuiltIns.booleanType to atomicIntFieldUpdaterClass,
        irBuiltIns.anyNType to atomicRefFieldUpdaterClass
    )
    ...
    private val ATOMIC_ARRAY_CLASSES: Map<String, IrClassSymbol> = mapOf(
        "AtomicIntArray" to atomicIntArrayClass,
        "AtomicBooleanArray" to atomicIntArrayClass,
        "AtomicLongArray" to atomicLongArrayClass,
        "AtomicArray" to atomicRefArrayClass
    )
    ...
    private val ATOMIC_ARRAY_TYPES: Set<IrClassSymbol> = setOf(atomicIntArrayClass, atomicLongArrayClass, atomicRefArrayClass)

    private val ATOMIC_FIELD_UPDATER_TYPES: Set<IrClassSymbol> = setOf(atomicIntFieldUpdaterClass, atomicLongFieldUpdaterClass, atomicRefFieldUpdaterClass)

    fun getFieldUpdaterClass(valueType: IrType): IrClassSymbol {
        return if (valueType.isPrimitiveType())
            UPDATER_CLASSES[valueType] ?: error("No corresponding atomic updater found for this value type ${valueType.render()} ")
        else atomicRefFieldUpdaterClass
    }

    fun getAtomicArrayClassByAtomicfuArrayName(atomicArrayType: IrType): IrClassSymbol =
        ATOMIC_ARRAY_CLASSES[atomicArrayType.classFqName?.shortName()?.asString()] ?: error("No corresponding atomic array class found for this value type ${atomicArrayType.render()} ")

    fun getAtomicArrayType(valueType: IrType): IrClassSymbol =
        VALUE_TYPE_TO_ATOMIC_ARRAY_CLASS[valueType] ?: error("No corresponding atomic array class found for this value type ${valueType.render()} ")

    fun getFieldUpdaterType(valueType: IrType) = getFieldUpdaterClass(valueType).defaultType
    ...
    fun getAtomicHandlerFunctionSymbol(atomicHandlerClass: IrClassSymbol, name: String): IrSimpleFunctionSymbol {
        // todo boolean to int
        return when (name) { // todo check if we can get class of updater via its type
